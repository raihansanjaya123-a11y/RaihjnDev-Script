-- GridSelector.lua
local UIS = game:GetService("UserInputService")  -- PENTING: definisikan di awal

local function CreateGridSelectorUI()
    -- Bungkus dengan pcall untuk tangkap error
    local success, err = pcall(function()
        if getgenv().AFGridGui then
            pcall(function() getgenv().AFGridGui:Destroy() end)
        end

        -- Warna (salin dari kode sebelumnya)
        ...
        -- Bersihkan koneksi jika frame dihapus
        ...
        local function MakeBtn(text, color, callback)
            ...
            btn.Position = UDim2.new((#btnRow:GetChildren()-1)*(1/3), 2, 0, 0)
            ...
            btn.MouseEnter:Connect(function()
                btn.BackgroundColor3 = btn.BackgroundColor3:Lerp(Color3.new(1,1,1),0.14)
            end)
        ...
        for _, child in pairs(btnRow:GetChildren()) do
...
getgenv().CreateGridSelectorUI = CreateGridSelectorUI-- GridSelector.lua
local UIS = game:GetService("UserInputService")  -- PENTING: definisikan di awal
